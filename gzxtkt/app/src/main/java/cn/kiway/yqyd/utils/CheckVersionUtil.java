...
public class CheckVersionUtil {
    ...
    public static String yqydPack = "gzxtkt";
    /**
     * 读取asset下面的版本号
     */
    public static String readFromAsset(Context context) {
        String res = "";
        try {
            InputStream in = context.getResources().getAssets().open("txkt_teacher" +
                    "/version/zip_ls.json");
    ...
    public static String readFileSdcard() {
        String res = "";
        if (new File(Environment.getExternalStorageDirectory() + "/txkt_teacher/version/zip_ls.json").exists()) {
            try {
                FileInputStream fin = new FileInputStream(Environment.getExternalStorageDirectory() +
                        "/txkt_teacher/version/zip_ls.json");
